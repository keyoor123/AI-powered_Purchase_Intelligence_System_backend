...
from app.schemas.agents import (
    ...
    YearlyAgentSettingsResponseSchema,
    YearlyAgentSettingsUpdateSchema
)
...
# ==========================================
# Yearly Report Agent API Endpoints
# ==========================================

yearly_router = APIRouter(prefix="/agents/yearly-report", tags=["Yearly Report Agent"])

@yearly_router.get("/settings", response_model=YearlyAgentSettingsResponseSchema)
async def get_yearly_agent_settings(
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Retrieves the Yearly Report Agent settings for the current user."""
    logger.info(f"Fetching yearly report agent settings for user: {user_id}")
    try:
        settings_col = db_manager.get_settings_collection()
        doc = await settings_col.find_one({
            "user_id": user_id, 
            "type": "agent_settings", 
            "value.agent_type": "yearly_report"
        })
            ...
            if value.get("is_enabled", False):
                ...
                if not next_run or (isinstance(next_run, datetime) and next_run <= now_dt):
                    ...
                    try:
                        ...
                        if candidate <= now_dt:
                            candidate = datetime(next_year + 1, month, day, 9, 0, 0)
                        value["next_run"] = candidate
                    except ValueError:
                        value["next_run"] = datetime(next_year + 1, month, 28, 9, 0, 0)
                    
                    await settings_col.update_one(
                        {"_id": doc["_id"]},
                        {"$set": {"value": value, "updated_at": datetime.utcnow()}}
                    )
            return YearlyAgentSettingsResponseSchema(**value)
        ...
        default_settings = {
            "agent_type": "yearly_report",
            "is_enabled": True,
            "schedule_config": {"month": 1, "day": 15},
            "delivery_emails": [{"email": default_email, "is_enabled": True}],
            "last_run": None,
            "next_run": None
        }
        ...
        # Sync with scheduler
        await agent_scheduler.sync_user_agent_job(user_id, "yearly_report")
        
        return YearlyAgentSettingsResponseSchema(**default_settings)
    except Exception as e:
        logger.error(f"Failed to retrieve yearly agent settings: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to retrieve yearly agent settings: {str(e)}"
        )

@yearly_router.put("/settings", response_model=YearlyAgentSettingsResponseSchema)
async def update_yearly_agent_settings(
    payload: YearlyAgentSettingsUpdateSchema,
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Updates the yearly agent toggle status or schedule."""
    logger.info(f"Updating yearly agent settings for user: {user_id}")
    try:
        settings_col = db_manager.get_settings_collection()
        doc = await settings_col.find_one({
            "user_id": user_id, 
        ...
        if not doc:
            await get_yearly_agent_settings(user_id, db)
            doc = await settings_col.find_one({
                "user_id": user_id, 
                "type": "agent_settings", 
                "value.agent_type": "yearly_report"
        ...
        if payload.month is not None:
            value["schedule_config"]["month"] = payload.month
        if payload.day is not None:
            value["schedule_config"]["day"] = payload.day
        ...
        logger.error(f"Failed to update yearly agent settings: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to update yearly agent settings: {str(e)}"
        )

@yearly_router.post("/emails", response_model=YearlyAgentSettingsResponseSchema)
async def add_yearly_recipient_email(
    payload: EmailRecipientAddSchema,
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Adds a new recipient email address to the yearly delivery list."""
    logger.info(f"Adding yearly recipient email for user {user_id}: {payload.email}")
    try:
        settings_col = db_manager.get_settings_collection()
        doc = await settings_col.find_one({
            "user_id": user_id, 
        ...
        logger.error(f"Failed to add yearly email recipient: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to add recipient email: {str(e)}"
        )

@yearly_router.put("/emails/toggle", response_model=YearlyAgentSettingsResponseSchema)
async def toggle_yearly_recipient_email(
    payload: EmailRecipientUpdateSchema,
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Enables or disables (toggles) a specific email recipient on the yearly distribution list."""
    logger.info(f"Toggling yearly email status for {payload.email}: {payload.is_enabled}")
    try:
        settings_col = db_manager.get_settings_collection()
        doc = await settings_col.find_one({
            "user_id": user_id, 
        ...
        logger.error(f"Failed to toggle yearly email recipient: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to toggle recipient email: {str(e)}"
        )

@yearly_router.delete("/emails/{email}", response_model=YearlyAgentSettingsResponseSchema)
async def remove_yearly_recipient_email(
    email: str,
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Deletes an email recipient address from the yearly delivery list."""
    logger.info(f"Removing yearly recipient email: {email}")
    try:
        settings_col = db_manager.get_settings_collection()
        doc = await settings_col.find_one({
            "user_id": user_id, 
        ...
        logger.error(f"Failed to remove yearly email recipient: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to remove recipient email: {str(e)}"
        )

@yearly_router.post("/trigger")
async def trigger_yearly_agent_manually(
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Manually triggers the Yearly Report Agent immediately."""
    logger.info(f"Manually triggering YearlyReportAgent for user: {user_id}")
    try:
        success = await yearly_report_agent.run(user_id)
        if success:
            return {"status": "success", "message": "Yearly Report compiled, saved, and dispatched successfully."}
        else:
            return {"status": "failed", "message": "Yearly Report failed to compile. Check logs or verify you have bills uploaded."}
    except Exception as e:
        logger.error(f"Manual yearly agent trigger failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Agent trigger failed: {str(e)}"
        )

@yearly_router.get("/logs")
async def get_yearly_execution_logs(
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Retrieves execution audit logs for this user's yearly report agent."""
    logger.info(f"Fetching yearly agent execution logs for user: {user_id}")
    try:
        cursor = db["agent_execution_logs"].find({"user_id": user_id, "agent_type": "yearly_report"}).sort("run_at", -1)
        logs = await cursor.to_list(length=100)
        ...
        logger.error(f"Failed to retrieve yearly execution logs: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to fetch execution logs: {str(e)}"
        )

@yearly_router.get("/reports/{file_id}")
async def download_yearly_report_pdf(
    file_id: str,
    user_id: str = Depends(get_current_user_id),
    db: AsyncIOMotorDatabase = Depends(get_database)
):
    """Downloads a generated yearly PDF report from GridFS."""
    logger.info(f"Downloading yearly report PDF from GridFS: {file_id}")
    try:
        fs = AsyncIOMotorGridFSBucket(db)
        cursor = fs.find({"_id": ObjectId(file_id)})
        files = await cursor.to_list(length=1)
        ...
        logger.error(f"Failed to download yearly report PDF: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to download report PDF: {str(e)}"
        )
